...
class _PagosFormViewState extends State<PagosFormView> {
  ...
  void initState() {
    ...
    if (pago != null) {
      ...
      _hora = TimeOfDay(hour: pago.fechapago.hour, minute: pago.fechapago.minute);
  ...
  Future<void> _loadCuentas() async {
    ...
    try {
      final List<CuentaBancaria> cuentas =
          await CuentaBancaria.getCuentas();
      if (!mounted) {
        return;
      }
      final Pago? pago = widget.pago;
      if (pago != null &&
          pago.idcuenta != null &&
          cuentas.every((CuentaBancaria cuenta) => cuenta.id != pago.idcuenta)) {
        cuentas.insert(
          0,
          CuentaBancaria(
            id: pago.idcuenta!,
